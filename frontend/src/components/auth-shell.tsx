...
export function AuthShell({
  title,
  description,
  panelBadge = "SSO terpusat & aman",
  panelTitle = "Satu identitas untuk seluruh layanan organisasi.",
  panelDescription = "Gunakan satu akun untuk mengakses semua layanan digital PC IPNU IPPNU Kabupaten Magetan.",
  children,
}: {
  title: string;
  description: string;
  ...
  return (
    <main className="auth-shell relative isolate min-h-svh w-full overflow-x-clip bg-background lg:bg-muted/35">
      ...
      <div className="relative mx-auto flex min-h-svh w-full min-w-0 items-start lg:items-center lg:justify-center lg:p-8">
        <div className="auth-shell-frame grid w-full min-w-0 grid-cols-[minmax(0,1fr)] bg-background lg:min-h-[42rem] lg:max-w-6xl lg:grid-cols-2 lg:overflow-hidden lg:rounded-3xl lg:shadow-2xl lg:shadow-primary/8 lg:ring-1 lg:ring-foreground/10">
          <section className="relative hidden min-w-0 flex-col justify-between overflow-hidden bg-primary p-10 text-primary-foreground lg:flex xl:p-12">
            ...
            <p className="relative text-xs text-primary-foreground/60">PelajarNU Magetan ID · Single Sign-On</p>
          </section>
          <div className="auth-shell-form flex w-full min-w-0 max-w-full items-start px-5 pt-[max(1.5rem,env(safe-area-inset-top))] pb-[max(2rem,env(safe-area-inset-bottom))] sm:px-8 sm:py-10 lg:items-center lg:px-12 lg:py-12">
            <Card className="mx-auto w-full min-w-0 max-w-none gap-6 bg-transparent py-0 ring-0 shadow-none">
